...
async function loadLeads(){
  try{
    ...
    try{ document.getElementById('leads-accepted').textContent = leads.filter(function(l){ return l.status==='accepted'; }).length; }catch(e){}
    ...
    var html = leads.length ? leads.map(function(l){
      var lid=l.id, lstat=l.status||'new';
      var aBtn = (lstat==='new'||lstat==='pending')
        ? '<button onclick="acceptLead(\''+lid+'\')" class="btn btn-success btn-xs">Accept</button> ' : '';
      var dBtn = lstat!=='declined'
        ? '<button onclick="declineLead(\''+lid+'\')" class="btn btn-outline btn-xs" style="color:var(--red);border-color:var(--red);">Decline</button> ' : '';
      var eBtn = '<button onclick="editLead(\''+lid+'\')" class="btn btn-outline btn-xs">Edit</button> ';
      var xBtn = '<button onclick="deleteLead(\''+lid+'\')" class="btn btn-outline btn-xs" style="color:var(--red);border-color:var(--red);">Del</button>';
      return '<tr><td><b>'+(l.name||l.client_name||'--')+'</b><br><span style="font-size:11px;color:var(--tx3);">'+(l.phone||l.email||'')+'</span></td>'
        +'<td>'+(l.service||l.service_type||'--')+'</td>'
        +'<td>'+(l.address||l.location||'--')+'</td>'
        +'<td>'+fmtD(l.created_at)+'</td>'
        +'<td>'+bdg(lstat)+'</td>'
        +'<td style="white-space:nowrap;">'+aBtn+dBtn+eBtn+xBtn+'</td></tr>';
...
async function acceptLead(id){
  try{
    var r = await fetch(SB_CS_URL+'/rest/v1/leads?id=eq.'+id+'&select=*', {headers:{'apikey':SB_CS_KEY,'Authorization':'Bearer '+SB_CS_KEY}});
    var data = await r.json(); var l = data[0]||{};
    await fetch(SB_CS_URL+'/rest/v1/leads?id=eq.'+id, {method:'PATCH',headers:{'apikey':SB_CS_KEY,'Authorization':'Bearer '+SB_CS_KEY,'Content-Type':'application/json'},body:JSON.stringify({status:'accepted'})});
    await sb.from('requests').insert([{client_name:l.name||'',service:l.service||'',phone:l.phone||'',address:l.address||'',status:'New',source:'CleanSaver AI'}]);
    if(l.phone){ var msg='Hi '+(l.name||'')+'! CleanPro TX accepted your request. We will contact you shortly. '; window.open('https://wa.me/'+l.phone.replace(/\D/g,'')+'?text='+encodeURIComponent(msg),'_blank'); }
    loadLeads();
  }catch(err){ console.error('acceptLead error:',err); }
}
async function declineLead(id){
  await fetch(SB_CS_URL+'/rest/v1/leads?id=eq.'+id, {method:'PATCH',headers:{'apikey':SB_CS_KEY,'Authorization':'Bearer '+SB_CS_KEY,'Content-Type':'application/json'},body:JSON.stringify({status:'declined'})});
  loadLeads();
}
...
async function loadJobs(statusFilter){
  ...
  // Jobs section shows TODAY and PAST jobs only. Future jobs are in Calendar.
  var todayEnd = new Date(); todayEnd.setHours(23,59,59,999);
  var rows = allJ.filter(function(j){
    if(!j.scheduled_date) return true; // no date = show it
...
async function loadQuotes(statusFilter){
  ...
  var html = rows.length ? rows.map(function(q){
    ...
    var actions = '';
    if(q.status==='Draft'){
      actions = '<button onclick="sendQuote(\''+qid+'\')" class="btn btn-primary btn-xs">Send</button> ';
    } else if(q.status==='Awaiting'||q.status==='Sent'){
      actions = '<button onclick="approveQuote(\''+qid+'\')" class="btn btn-success btn-xs">Approve</button> ';
    } else if(q.status==='Approved'){
      actions = '<button onclick="convertQuoteToJob(\''+qid+'\')" class="btn btn-success btn-xs">→ Job</button> ';
    }
...
async function completeAndInvoice(jobId){
  ...
  showConfirm('Mark job for '+job.client_name+' as Completed and create invoice?', async function(){
    ...
    // Auto-send review request via WhatsApp after job completion
    setTimeout(function(){
      var clientRes = _allClients.find(function(c){ return c.name===job.client_name; });
      var phone = clientRes&&clientRes.phone ? clientRes.phone.replace(/\D/g,'') : '';
      if(phone){
        showConfirm('Send review request to '+job.client_name+' via WhatsApp?', function(){
          var msg = 'Hi '+job.client_name+'! Thank you for choosing CleanPro TX! We hope your home is sparkling clean! Would you mind leaving us a Google review? It only takes 1 minute and means a lot to us! https://business.google.com Thank you! - Maria ';
...
async function approveQuote(id){
  ...
  showAlert('Estimate approved! You can now convert it to a job.');
  loadQuotes();
}
...
async function loadAcct(){
  try{
    ...
    var mhtml = months.map(function(mo,i){
      ...
      return '<tr><td><b>'+mo+'</b></td>'
        +'<td style="color:var(--green);"><b>'+fmt(mrev[i])+'</b></td>'
        +'<td style="color:var(--red);">'+fmt(mexp[i])+'</td>'
        +'<td style="color:'+(mp>=0?'var(--green)':'var(--red)')+'"><b>'+fmt(mp)+'</b></td></tr>';
    }).filter(Boolean).join('');
