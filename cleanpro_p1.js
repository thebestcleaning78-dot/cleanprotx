...
function bdg(s){
  var map={
    ...
    'pending':'b-new','To confirm':'b-progress'
  };
...
function mapsBtn(addr){
  if(!addr)return '';
  return '<button onclick="window.open(\'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(addr+' Lubbock TX')+'\')" class="btn btn-outline btn-xs">Map</button>';
}
function smsBtn(id){
...
var _bizId = null; // current business_id

// Get business_id for current user
async function getBizId(){
  if(_bizId) return _bizId;
  var res = await sb.from('businesses').select('id,business_name,plan').limit(1).single();
  if(res.data){
    _bizId = res.data.id;
    // Show business name in sidebar
    try{ document.querySelector('.logo-text').textContent = res.data.business_name; }catch(e){}
    try{ document.querySelector('.logo-sub').textContent = res.data.plan+' Plan'; }catch(e){}
  }
  return _bizId;
}
...
sb.auth.getUser().then(function(res){
  ...
  if(user){
    var name = (user.user_metadata && user.user_metadata.full_name) || user.email || 'Maria';
...
function nav(id, el){
  ...
  var titles = {
    ...
    accounting:'Reports', gallery:'Gallery', timesheets:'Timesheets', apps:'Apps'
  };
...
function loadSec(id){
  getBizId(); // ensure business_id is loaded
  if(id==='schedule')    loadSchedule();
  else if(id==='calendar')   {} // Google Calendar - no load needed
  ...
  else if(id==='marketing')   {} // static page
...
function filterClients(q){
  var rows = document.querySelectorAll('#clients-table tr');
...
function filterJobs(q){
  var rows = document.querySelectorAll('#jobs-table tr');
  rows.forEach(function(r){
    r.style.display = r.textContent.toLowerCase().includes(q.toLowerCase()) ? '' : 'none';
  });
}
function filterJobStatus(s){
  loadJobs(s);
}
function filterInvoices(q){
  var rows = document.querySelectorAll('#invoices-table tr');
  rows.forEach(function(r){
    r.style.display = r.textContent.toLowerCase().includes(q.toLowerCase()) ? '' : 'none';
  });
}
function filterInvoiceStatus(s){
  loadInvoices(s);
}
function filterQuoteStatus(s){
  loadQuotes(s);
}
